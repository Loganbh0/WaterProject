...
const AdminProjectsPage = () => {
  ...
  useEffect(() => {
    const loadProjects = async () => {
      try {
        ...
        setTotalPages(Math.ceil(data.totalProjects / pageSize));
      } catch (err) {
  ...
  return (
    <div>
      ...
      {!showForm && (
        <button className="btn btn-success mb-3" onClick={() => setShowForm(true)}>Add New Project</button>
      )}
        ...
        <tbody>
          {projects.map((p) => (
            <tr key={p.projectId}>
              ...
              <td>
                <button className="btn btn-primary btn-small w-100 mb-1"
                  onClick={() => setEditingProject(p)}
                >
                  Edit
                </button>
                <button className="btn btn-danger btn-small w-100"
